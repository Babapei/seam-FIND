#!/usr/bin/env python3
"""
训练 PointNet 焊缝二类分割。数据由 seam_localization/pointcloud_dataset 生成。
"""
import os
import sys
import argparse
...
from seam_localization.pointnet_seam.model import PointNetSeg
from seam_localization.pointnet_seam.data import SeamPointCloudDataset


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", type=str, default=None, help="目录含 seam_train.npy / seam_val.npy")
    ...
    if args.data_dir is None:
        args.data_dir = os.path.join(os.path.dirname(_here), "pointcloud_data")
    ...
    if not os.path.isfile(train_npy) or not os.path.isfile(val_npy):
        print("未找到数据，请先运行: python -m seam_localization.pointcloud_dataset")
        print("  或: python seam_localization/pointcloud_dataset.py --num_train 200 --num_val 50")
        sys.exit(1)
    ...
    for epoch in range(args.epochs):
        ...
        for pts, lab in train_loader:
            ...
            opt.zero_grad()
            logits = model(pts)
            loss = criterion(logits.view(-1, 2), lab.view(-1))
            loss.backward()
            opt.step()
